...
if len(sys.argv) == 6:
    ...
    outdir = sys.argv[4]
    generateout_dir = sys.argv[5]
    if not os.path.exists(generateout_dir):
        os.mkdir(generateout_dir)
else:
    print("Usage:" + sys.argv[0] + "<irrigation: T or F> <use_region_mask: T or F> <annual_data: T or F> <gridmean_dir_without_slash> <outputdir_without_slash>" )
    exit()
...
cellid_file = current_dir + "/UM_calibration_cellid_list.txt"

if annual_data:
    grid_mean_data = "irrig" + irrigation + "crop_all_annual_avg_withfraction_area.txt"
else:
    grid_mean_data = "irrig" + irrigation + "crop_all_month_avg_withfraction_area.txt"
...
if os.path.exists(grid_mean_data):
    ...
    print("File:" + grid_mean_data + " not exist!\n")
    exit()
