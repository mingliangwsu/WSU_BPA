#!/bin/bash
outfilename=Sweet_corn_OFoot_Generic.crp
...
sed -i 's/^C_species=.*/C_species=C4/g' $outfilename
...
sed -i 's/^TUE_at_1pKa_VPD=.*/TUE_at_1pKa_VPD=7.5/g' $outfilename
sed -i 's/^TUE_at_1pKa_VPD_veg=.*/TUE_at_1pKa_VPD_veg=7.5/g' $outfilename
...
sed -i 's/^max_water_uptake=.*/max_water_uptake=14/g' $outfilename
...
sed -i 's/^peak_LAI=.*/peak_LAI=880/g' $outfilename
sed -i 's/^max_root_depth=.*/max_root_depth/g' $outfilename
sed -i ':a;N;$!ba;s/max_root_depth/max_root_depth=880/1' $outfilename
sed -i 's/^flowering=.*/flowering=900/g' $outfilename
sed -i 's/^filling=.*/filling=950/g' $outfilename
sed -i 's/^senescence (begin)=.*/senescence (begin)=1080/g' $outfilename
sed -i 's/^maturity=.*/maturity=1900/g' $outfilename
sed -i 's/^full_senescence=.*/full_senescence=2000/g' $outfilename
sed -i 's/^thermal_response=.*/thermal_response=linear/g' $outfilename
sed -i 's/^base_temp=.*/base_temp=5/g' $outfilename
sed -i 's/^cutoff_temp=.*/cutoff_temp=30/g' $outfilename
...
sed -i ':a;N;$!ba;s/max_root_depth/max_root_depth=2/2' $outfilename
...
sed -i 's/^max_canopy_height=.*/max_canopy_height=2/g' $outfilename
...
sed -i 's/^unstressed=.*/unstressed=0.5/g' $outfilename
...
sed -i 's/^alpha=.*/alpha=0.0038/g' $outfilename
sed -i 's/^theta=.*/theta=0.977/g' $outfilename
sed -i 's/^growth_ratio_asymtotic=.*/growth_ratio_asymtotic=1.063/g' $outfilename
